...
export const ImageViewBlock: React.FC<NodeViewProps> = ({
  ...
  React.useEffect(() => {
    const handleImage = async () => {
      ...
      const { uploadFn } = imageExtension?.options as ImageExtensionOptions
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          maxWidth: `min(${String(maxWidth)}px, 100%)`,
          width: currentWidth,
          maxHeight: MAX_HEIGHT,
          aspectRatio: `${String(imageState.naturalSize.width)} / ${String(
            imageState.naturalSize.height
          )}`
        }}
      >
        <div
            ...
            <div className="relative h-full">
              ...
              >
                <img
                  ...
                  style={{
                    maxWidth: `min(100%, ${String(maxWidth)}px)`,
                    minWidth: `${String(MIN_WIDTH)}px`,
                    maxHeight: String(MAX_HEIGHT)
                  }}
                  width={currentWidth}
                  ...
                  alt={(node.attrs as ImageAttributes).alt || ''}
                  title={(node.attrs as ImageAttributes).title || ''}
                  id={(node.attrs as ImageAttributes).id}
                />
              </ControlledZoom>
            </div>
